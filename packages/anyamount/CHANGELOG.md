...
- 15a27e3: - **`anyamount.parse(text, { locale })`** — the other direction: `"1.999,00"` in German is `1999`, `"1 234,5"` in French is `1234.5`, Arabic-Indic digits count too. Separators and digits are read off `Intl`, whatever wraps the number (currency, `%`, whitespace) is ignored, a minus on either side or accounting parentheses negates, and anything that is not a number is `NaN` — never a throw. One rule beyond the locale: a lone separator followed by one or two digits is a decimal point (`"1.5"` in a German form is one and a half).
