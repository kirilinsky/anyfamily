    ...
    <>
      <div className="relative w-fit max-w-full rounded-xl border border-white/[0.07] bg-black/30 px-4 py-3.5 font-mono">
        <div className="flex min-h-9 flex-wrap items-center justify-center gap-x-1 gap-y-1.5 text-sm sm:text-base">
          <span className="shrink-0 text-accent">anymany</span>
          <span className="shrink-0 text-white/30">([</span>
          <input
            value={itemsRaw}
            onChange={(e) => setItemsRaw(e.target.value)}
            spellCheck={false}
            aria-label="list items, comma separated"
            style={{ width: `min(${itemsRaw.length + 2}ch, 100%)` }}
            className="h-8 max-w-full min-w-[8ch] shrink rounded-md border border-transparent px-1.5 font-mono text-sky-300 outline-none transition-colors hover:border-white/10 hover:bg-white/[0.05] focus:border-white/10 focus:bg-white/[0.05]"
          />
          <span className="shrink-0 text-white/30">], {"{"}</span>
